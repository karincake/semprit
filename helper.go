...
func reflectPointerValueFiller(fv reflect.Value, vk reflect.Kind, ftName, rvs string) error {
	// Allocate a new value of the pointer's *actual* element type (which may be
	// a named/custom type such as CustomTypeUint, not just a builtin), fill it
	// through the shared value filler, then assign its address. Building a
	// pointer to the hard-coded base type instead would panic in fv.Set because
	// e.g. *uint is not assignable to *CustomTypeUint.
	ev := reflect.New(fv.Type().Elem()).Elem()
	if err := reflectValueFiller(ev, vk, ftName, rvs); err != nil {
		return err
	}
	fv.Set(ev.Addr())
